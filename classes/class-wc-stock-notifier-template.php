...
class WC_Stock_Notifier_Template {

    public $template_url;

    public function __construct() {
        $this->template_url = 'wc-stock-notifier/';
    }
    ...
    public function locate_template( $template_name, $template_path = '', $default_path = '' ) {
        global $woocommerce, $WC_Stock_Notifier;
        $default_path = apply_filters( 'template_path', $default_path );
        if ( ! $template_path ) {
            $template_path = $this->template_url;
        }
        if ( ! $default_path ) {
            $default_path = $WC_Stock_Notifier->plugin_path . 'templates/';
        }
    ...
    public function get_template_part( $slug, $name = '' ) {
        ...
        if ( ! $template && $name && file_exists( $WC_Stock_Notifier->plugin_path . "templates/{$slug}-{$name}.php" ) )
            $template = $WC_Stock_Notifier->plugin_path . "templates/{$slug}-{$name}.php";
